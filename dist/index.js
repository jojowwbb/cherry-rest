...
var UNDEFINED_PATTERN = /^undefined | undefined$|^[^(]* undefined /i; //rest对象

var Cherry = {}; //fetch基础配置

var fetchOption = {}; //全局配置
...
var filterOptions = {
  over: function over(source) {
    ...
    if (type === '[object Object]') {
      ...
      result = source.map(function (data) {
        return format(data);
      });
    }
...
function debounceCreator(fun, delay) {
  return function (args) {
    //获取函数的作用域和变量
    var that = this;
    var _args = args; //每次事件被触发，都会清除当前的timeer，然后重写设置超时调用
    ...
    value: function _load(fetchOption) {
      ...
      if (commonOption.noFetch) {
        ...
        return _prms.then(commonOption.formatter).then(function (res) {
          ...
          var result = res;
          formatterArrays.map(function (formatter) {
